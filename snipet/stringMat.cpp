...
struct mat {
  ...
  mat() : m(vector<string>()) {}
  mat(int h, int w) : m(vector<string>(h, string(w))) {}
  mat(int h, int w, T d) : m(vector<string>(h, string(w, d))) {}
  // 添字演算子
  string operator[](const int i) const { return m[i]; }  //読み取り
  string& operator[](const int i) { return m[i]; }       //書き込み
  // 行数・列数
  int nrow = sz(m);
  int ncol = sz(m[0]);
  // 行列同士の演算
